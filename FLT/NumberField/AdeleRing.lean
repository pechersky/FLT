...
noncomputable instance : Algebra K (NumberField.AdeleRing (𝓞 L) L) :=
  RingHom.toAlgebra <| (algebraMap L (NumberField.AdeleRing (𝓞 L) L)).comp <| algebraMap K L

noncomputable abbrev tensorProductLinearEquivPi : AdeleRing (𝓞 K) K ⊗[K] L ≃ₗ[K]
...
```
instance :
    TopologicalSpace (NumberField.AdeleRing (𝓞 K) K ⊗[K] L) :=
  letI := TopologicalSpace.induced (algebraMap K (AdeleRing (𝓞 K) K)) inferInstance
  moduleTopology K _
```
...
coincide!
-/
instance : TopologicalSpace (NumberField.AdeleRing (𝓞 K) K ⊗[K] L) :=
...
noncomputable def tensorProductContinuousLinearEquivPi :
    AdeleRing (𝓞 K) K ⊗[K] L ≃L[K] (Fin (Module.finrank K L) → AdeleRing (𝓞 K) K) where
...
to using `AdeleRing (𝓞 K) K ⊗[K] L` is that it automatically gets
...
-/
def baseChange : (AdeleRing (𝓞 K) K ⊗[K] L) ≃A[K] AdeleRing (𝓞 L) L := sorry

variable {L}

theorem baseChange_tsum_apply_right (l : L) :
  baseChange K L (1 ⊗ₜ[K] l) = algebraMap L (AdeleRing (𝓞 L) L) l := sorry
